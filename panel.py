...
class PanelThread (threading.Thread):
    ...
    def run(self):
        ...
        print("    img {EXPRESSION} at {LOC} -> generates an image of the given expression and saves at {LOC}.")
                ...
                go.printExpression(x[5:],json.dumps(st.selectedcell))
                    ...
                    if len(expression)==2:
                            try:
                                    ...
                                    fail("The expression could not be parsed.")
                    else:
                            fail("The expression could not be parsed.")
